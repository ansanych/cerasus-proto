// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.1
// 	protoc        v3.12.4
// source: board_v2.proto

package cerasus_proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type PingService struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Service string `protobuf:"bytes,1,opt,name=service,proto3" json:"service,omitempty"`
	Ping    bool   `protobuf:"varint,2,opt,name=ping,proto3" json:"ping,omitempty"`
}

func (x *PingService) Reset() {
	*x = PingService{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PingService) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PingService) ProtoMessage() {}

func (x *PingService) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PingService.ProtoReflect.Descriptor instead.
func (*PingService) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{0}
}

func (x *PingService) GetService() string {
	if x != nil {
		return x.Service
	}
	return ""
}

func (x *PingService) GetPing() bool {
	if x != nil {
		return x.Ping
	}
	return false
}

type PingServicesReply struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Data []*PingService `protobuf:"bytes,1,rep,name=data,proto3" json:"data,omitempty"`
}

func (x *PingServicesReply) Reset() {
	*x = PingServicesReply{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *PingServicesReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PingServicesReply) ProtoMessage() {}

func (x *PingServicesReply) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PingServicesReply.ProtoReflect.Descriptor instead.
func (*PingServicesReply) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{1}
}

func (x *PingServicesReply) GetData() []*PingService {
	if x != nil {
		return x.Data
	}
	return nil
}

type SearchProductRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID      int64  `protobuf:"varint,1,opt,name=ID,proto3" json:"ID,omitempty"`
	Shop    string `protobuf:"bytes,2,opt,name=shop,proto3" json:"shop,omitempty"`
	Url     string `protobuf:"bytes,3,opt,name=url,proto3" json:"url,omitempty"`
	ShopID  string `protobuf:"bytes,4,opt,name=shopID,proto3" json:"shopID,omitempty"`
	Article string `protobuf:"bytes,5,opt,name=article,proto3" json:"article,omitempty"`
	Name    string `protobuf:"bytes,6,opt,name=name,proto3" json:"name,omitempty"`
}

func (x *SearchProductRequest) Reset() {
	*x = SearchProductRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *SearchProductRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchProductRequest) ProtoMessage() {}

func (x *SearchProductRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchProductRequest.ProtoReflect.Descriptor instead.
func (*SearchProductRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{2}
}

func (x *SearchProductRequest) GetID() int64 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *SearchProductRequest) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *SearchProductRequest) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *SearchProductRequest) GetShopID() string {
	if x != nil {
		return x.ShopID
	}
	return ""
}

func (x *SearchProductRequest) GetArticle() string {
	if x != nil {
		return x.Article
	}
	return ""
}

func (x *SearchProductRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type BoardProduct struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID      int64  `protobuf:"varint,1,opt,name=ID,proto3" json:"ID,omitempty"`
	Title   string `protobuf:"bytes,2,opt,name=title,proto3" json:"title,omitempty"`
	Comment string `protobuf:"bytes,3,opt,name=comment,proto3" json:"comment,omitempty"`
	Active  bool   `protobuf:"varint,4,opt,name=active,proto3" json:"active,omitempty"`
	Preview string `protobuf:"bytes,5,opt,name=preview,proto3" json:"preview,omitempty"`
	Updated string `protobuf:"bytes,6,opt,name=updated,proto3" json:"updated,omitempty"`
}

func (x *BoardProduct) Reset() {
	*x = BoardProduct{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardProduct) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardProduct) ProtoMessage() {}

func (x *BoardProduct) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardProduct.ProtoReflect.Descriptor instead.
func (*BoardProduct) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{3}
}

func (x *BoardProduct) GetID() int64 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *BoardProduct) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *BoardProduct) GetComment() string {
	if x != nil {
		return x.Comment
	}
	return ""
}

func (x *BoardProduct) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *BoardProduct) GetPreview() string {
	if x != nil {
		return x.Preview
	}
	return ""
}

func (x *BoardProduct) GetUpdated() string {
	if x != nil {
		return x.Updated
	}
	return ""
}

type BoardShopProduct struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Shop        string       `protobuf:"bytes,1,opt,name=shop,proto3" json:"shop,omitempty"`
	ShopProduct *ShopProduct `protobuf:"bytes,2,opt,name=shopProduct,proto3" json:"shopProduct,omitempty"`
	Files       int32        `protobuf:"varint,3,opt,name=files,proto3" json:"files,omitempty"`
	IsSales     bool         `protobuf:"varint,4,opt,name=isSales,proto3" json:"isSales,omitempty"`
	IsOrders    bool         `protobuf:"varint,5,opt,name=isOrders,proto3" json:"isOrders,omitempty"`
}

func (x *BoardShopProduct) Reset() {
	*x = BoardShopProduct{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardShopProduct) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardShopProduct) ProtoMessage() {}

func (x *BoardShopProduct) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardShopProduct.ProtoReflect.Descriptor instead.
func (*BoardShopProduct) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{4}
}

func (x *BoardShopProduct) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *BoardShopProduct) GetShopProduct() *ShopProduct {
	if x != nil {
		return x.ShopProduct
	}
	return nil
}

func (x *BoardShopProduct) GetFiles() int32 {
	if x != nil {
		return x.Files
	}
	return 0
}

func (x *BoardShopProduct) GetIsSales() bool {
	if x != nil {
		return x.IsSales
	}
	return false
}

func (x *BoardShopProduct) GetIsOrders() bool {
	if x != nil {
		return x.IsOrders
	}
	return false
}

type BoardProductPricer struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Shop   string `protobuf:"bytes,1,opt,name=shop,proto3" json:"shop,omitempty"`
	Active bool   `protobuf:"varint,2,opt,name=active,proto3" json:"active,omitempty"`
}

func (x *BoardProductPricer) Reset() {
	*x = BoardProductPricer{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardProductPricer) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardProductPricer) ProtoMessage() {}

func (x *BoardProductPricer) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardProductPricer.ProtoReflect.Descriptor instead.
func (*BoardProductPricer) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{5}
}

func (x *BoardProductPricer) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *BoardProductPricer) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

type BoardProductCounterQueueData struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ShopProductID int64 `protobuf:"varint,1,opt,name=shopProductID,proto3" json:"shopProductID,omitempty"`
	Count         int32 `protobuf:"varint,2,opt,name=count,proto3" json:"count,omitempty"`
	MpCount       int32 `protobuf:"varint,3,opt,name=mpCount,proto3" json:"mpCount,omitempty"`
}

func (x *BoardProductCounterQueueData) Reset() {
	*x = BoardProductCounterQueueData{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardProductCounterQueueData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardProductCounterQueueData) ProtoMessage() {}

func (x *BoardProductCounterQueueData) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardProductCounterQueueData.ProtoReflect.Descriptor instead.
func (*BoardProductCounterQueueData) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{6}
}

func (x *BoardProductCounterQueueData) GetShopProductID() int64 {
	if x != nil {
		return x.ShopProductID
	}
	return 0
}

func (x *BoardProductCounterQueueData) GetCount() int32 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *BoardProductCounterQueueData) GetMpCount() int32 {
	if x != nil {
		return x.MpCount
	}
	return 0
}

type BoardProductCounter struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Shop              string                          `protobuf:"bytes,1,opt,name=shop,proto3" json:"shop,omitempty"`
	Active            bool                            `protobuf:"varint,2,opt,name=active,proto3" json:"active,omitempty"`
	Count             int32                           `protobuf:"varint,3,opt,name=count,proto3" json:"count,omitempty"`
	ShopProductsCount []*BoardProductCounterQueueData `protobuf:"bytes,4,rep,name=shopProductsCount,proto3" json:"shopProductsCount,omitempty"`
}

func (x *BoardProductCounter) Reset() {
	*x = BoardProductCounter{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardProductCounter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardProductCounter) ProtoMessage() {}

func (x *BoardProductCounter) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardProductCounter.ProtoReflect.Descriptor instead.
func (*BoardProductCounter) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{7}
}

func (x *BoardProductCounter) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *BoardProductCounter) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *BoardProductCounter) GetCount() int32 {
	if x != nil {
		return x.Count
	}
	return 0
}

func (x *BoardProductCounter) GetShopProductsCount() []*BoardProductCounterQueueData {
	if x != nil {
		return x.ShopProductsCount
	}
	return nil
}

type BoardProductData struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Product      *BoardProduct          `protobuf:"bytes,1,opt,name=product,proto3" json:"product,omitempty"`
	Company      *Company               `protobuf:"bytes,2,opt,name=company,proto3" json:"company,omitempty"`
	Purchase     float32                `protobuf:"fixed32,3,opt,name=purchase,proto3" json:"purchase,omitempty"`
	Brand        *Brand                 `protobuf:"bytes,4,opt,name=brand,proto3" json:"brand,omitempty"`
	ShopProducts []*BoardShopProduct    `protobuf:"bytes,5,rep,name=shopProducts,proto3" json:"shopProducts,omitempty"`
	PricerData   []*BoardProductPricer  `protobuf:"bytes,6,rep,name=pricerData,proto3" json:"pricerData,omitempty"`
	CounterData  []*BoardProductCounter `protobuf:"bytes,7,rep,name=counterData,proto3" json:"counterData,omitempty"`
}

func (x *BoardProductData) Reset() {
	*x = BoardProductData{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardProductData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardProductData) ProtoMessage() {}

func (x *BoardProductData) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardProductData.ProtoReflect.Descriptor instead.
func (*BoardProductData) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{8}
}

func (x *BoardProductData) GetProduct() *BoardProduct {
	if x != nil {
		return x.Product
	}
	return nil
}

func (x *BoardProductData) GetCompany() *Company {
	if x != nil {
		return x.Company
	}
	return nil
}

func (x *BoardProductData) GetPurchase() float32 {
	if x != nil {
		return x.Purchase
	}
	return 0
}

func (x *BoardProductData) GetBrand() *Brand {
	if x != nil {
		return x.Brand
	}
	return nil
}

func (x *BoardProductData) GetShopProducts() []*BoardShopProduct {
	if x != nil {
		return x.ShopProducts
	}
	return nil
}

func (x *BoardProductData) GetPricerData() []*BoardProductPricer {
	if x != nil {
		return x.PricerData
	}
	return nil
}

func (x *BoardProductData) GetCounterData() []*BoardProductCounter {
	if x != nil {
		return x.CounterData
	}
	return nil
}

type UpdateBoardCompanyServiceRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Active    bool   `protobuf:"varint,1,opt,name=active,proto3" json:"active,omitempty"`
	End       string `protobuf:"bytes,2,opt,name=end,proto3" json:"end,omitempty"`
	Limit     int32  `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	Shop      string `protobuf:"bytes,4,opt,name=shop,proto3" json:"shop,omitempty"`
	CompanyID int64  `protobuf:"varint,5,opt,name=companyID,proto3" json:"companyID,omitempty"`
}

func (x *UpdateBoardCompanyServiceRequest) Reset() {
	*x = UpdateBoardCompanyServiceRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBoardCompanyServiceRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBoardCompanyServiceRequest) ProtoMessage() {}

func (x *UpdateBoardCompanyServiceRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBoardCompanyServiceRequest.ProtoReflect.Descriptor instead.
func (*UpdateBoardCompanyServiceRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{9}
}

func (x *UpdateBoardCompanyServiceRequest) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *UpdateBoardCompanyServiceRequest) GetEnd() string {
	if x != nil {
		return x.End
	}
	return ""
}

func (x *UpdateBoardCompanyServiceRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *UpdateBoardCompanyServiceRequest) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *UpdateBoardCompanyServiceRequest) GetCompanyID() int64 {
	if x != nil {
		return x.CompanyID
	}
	return 0
}

type UpdateBoardCompanyRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID     int64 `protobuf:"varint,1,opt,name=ID,proto3" json:"ID,omitempty"`
	Active bool  `protobuf:"varint,2,opt,name=active,proto3" json:"active,omitempty"`
}

func (x *UpdateBoardCompanyRequest) Reset() {
	*x = UpdateBoardCompanyRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateBoardCompanyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateBoardCompanyRequest) ProtoMessage() {}

func (x *UpdateBoardCompanyRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateBoardCompanyRequest.ProtoReflect.Descriptor instead.
func (*UpdateBoardCompanyRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{10}
}

func (x *UpdateBoardCompanyRequest) GetID() int64 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *UpdateBoardCompanyRequest) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

type BoardPricerShop struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Shop           string `protobuf:"bytes,1,opt,name=shop,proto3" json:"shop,omitempty"`
	Active         bool   `protobuf:"varint,2,opt,name=active,proto3" json:"active,omitempty"`
	Start          string `protobuf:"bytes,3,opt,name=start,proto3" json:"start,omitempty"`
	End            string `protobuf:"bytes,4,opt,name=end,proto3" json:"end,omitempty"`
	Period         int32  `protobuf:"varint,5,opt,name=period,proto3" json:"period,omitempty"`
	ProductsLimit  int32  `protobuf:"varint,6,opt,name=productsLimit,proto3" json:"productsLimit,omitempty"`
	ProductsActive int32  `protobuf:"varint,7,opt,name=productsActive,proto3" json:"productsActive,omitempty"`
	QueueStatus    int32  `protobuf:"varint,8,opt,name=queueStatus,proto3" json:"queueStatus,omitempty"`
	QueueError     bool   `protobuf:"varint,9,opt,name=queueError,proto3" json:"queueError,omitempty"`
	QueueUpdated   string `protobuf:"bytes,10,opt,name=queueUpdated,proto3" json:"queueUpdated,omitempty"`
}

func (x *BoardPricerShop) Reset() {
	*x = BoardPricerShop{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardPricerShop) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardPricerShop) ProtoMessage() {}

func (x *BoardPricerShop) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardPricerShop.ProtoReflect.Descriptor instead.
func (*BoardPricerShop) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{11}
}

func (x *BoardPricerShop) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *BoardPricerShop) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *BoardPricerShop) GetStart() string {
	if x != nil {
		return x.Start
	}
	return ""
}

func (x *BoardPricerShop) GetEnd() string {
	if x != nil {
		return x.End
	}
	return ""
}

func (x *BoardPricerShop) GetPeriod() int32 {
	if x != nil {
		return x.Period
	}
	return 0
}

func (x *BoardPricerShop) GetProductsLimit() int32 {
	if x != nil {
		return x.ProductsLimit
	}
	return 0
}

func (x *BoardPricerShop) GetProductsActive() int32 {
	if x != nil {
		return x.ProductsActive
	}
	return 0
}

func (x *BoardPricerShop) GetQueueStatus() int32 {
	if x != nil {
		return x.QueueStatus
	}
	return 0
}

func (x *BoardPricerShop) GetQueueError() bool {
	if x != nil {
		return x.QueueError
	}
	return false
}

func (x *BoardPricerShop) GetQueueUpdated() string {
	if x != nil {
		return x.QueueUpdated
	}
	return ""
}

type BoardCounterData struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Active bool   `protobuf:"varint,1,opt,name=active,proto3" json:"active,omitempty"`
	Start  string `protobuf:"bytes,2,opt,name=start,proto3" json:"start,omitempty"`
	End    string `protobuf:"bytes,3,opt,name=end,proto3" json:"end,omitempty"`
}

func (x *BoardCounterData) Reset() {
	*x = BoardCounterData{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardCounterData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardCounterData) ProtoMessage() {}

func (x *BoardCounterData) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardCounterData.ProtoReflect.Descriptor instead.
func (*BoardCounterData) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{12}
}

func (x *BoardCounterData) GetActive() bool {
	if x != nil {
		return x.Active
	}
	return false
}

func (x *BoardCounterData) GetStart() string {
	if x != nil {
		return x.Start
	}
	return ""
}

func (x *BoardCounterData) GetEnd() string {
	if x != nil {
		return x.End
	}
	return ""
}

type BoardCompanyData struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Company     *Company           `protobuf:"bytes,1,opt,name=company,proto3" json:"company,omitempty"`
	ShopData    []*ShopData        `protobuf:"bytes,2,rep,name=shopData,proto3" json:"shopData,omitempty"`
	PricerData  []*BoardPricerShop `protobuf:"bytes,3,rep,name=pricerData,proto3" json:"pricerData,omitempty"`
	CounterData *BoardCounterData  `protobuf:"bytes,4,opt,name=counterData,proto3" json:"counterData,omitempty"`
}

func (x *BoardCompanyData) Reset() {
	*x = BoardCompanyData{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardCompanyData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardCompanyData) ProtoMessage() {}

func (x *BoardCompanyData) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardCompanyData.ProtoReflect.Descriptor instead.
func (*BoardCompanyData) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{13}
}

func (x *BoardCompanyData) GetCompany() *Company {
	if x != nil {
		return x.Company
	}
	return nil
}

func (x *BoardCompanyData) GetShopData() []*ShopData {
	if x != nil {
		return x.ShopData
	}
	return nil
}

func (x *BoardCompanyData) GetPricerData() []*BoardPricerShop {
	if x != nil {
		return x.PricerData
	}
	return nil
}

func (x *BoardCompanyData) GetCounterData() *BoardCounterData {
	if x != nil {
		return x.CounterData
	}
	return nil
}

type BoardCompaniesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Page  int32 `protobuf:"varint,1,opt,name=page,proto3" json:"page,omitempty"`
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
}

func (x *BoardCompaniesRequest) Reset() {
	*x = BoardCompaniesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[14]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *BoardCompaniesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BoardCompaniesRequest) ProtoMessage() {}

func (x *BoardCompaniesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[14]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BoardCompaniesRequest.ProtoReflect.Descriptor instead.
func (*BoardCompaniesRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{14}
}

func (x *BoardCompaniesRequest) GetPage() int32 {
	if x != nil {
		return x.Page
	}
	return 0
}

func (x *BoardCompaniesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type ReQueueRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Auth   *Auth  `protobuf:"bytes,1,opt,name=auth,proto3" json:"auth,omitempty"`
	ID     int64  `protobuf:"varint,2,opt,name=ID,proto3" json:"ID,omitempty"`
	Code   string `protobuf:"bytes,3,opt,name=code,proto3" json:"code,omitempty"`
	Msv    string `protobuf:"bytes,4,opt,name=msv,proto3" json:"msv,omitempty"`
	Action string `protobuf:"bytes,5,opt,name=action,proto3" json:"action,omitempty"`
}

func (x *ReQueueRequest) Reset() {
	*x = ReQueueRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[15]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ReQueueRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReQueueRequest) ProtoMessage() {}

func (x *ReQueueRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[15]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReQueueRequest.ProtoReflect.Descriptor instead.
func (*ReQueueRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{15}
}

func (x *ReQueueRequest) GetAuth() *Auth {
	if x != nil {
		return x.Auth
	}
	return nil
}

func (x *ReQueueRequest) GetID() int64 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *ReQueueRequest) GetCode() string {
	if x != nil {
		return x.Code
	}
	return ""
}

func (x *ReQueueRequest) GetMsv() string {
	if x != nil {
		return x.Msv
	}
	return ""
}

func (x *ReQueueRequest) GetAction() string {
	if x != nil {
		return x.Action
	}
	return ""
}

type QueuesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Auth      *Auth `protobuf:"bytes,1,opt,name=auth,proto3" json:"auth,omitempty"`
	CompanyID int64 `protobuf:"varint,2,opt,name=companyID,proto3" json:"companyID,omitempty"`
}

func (x *QueuesRequest) Reset() {
	*x = QueuesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[16]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueuesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueuesRequest) ProtoMessage() {}

func (x *QueuesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[16]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueuesRequest.ProtoReflect.Descriptor instead.
func (*QueuesRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{16}
}

func (x *QueuesRequest) GetAuth() *Auth {
	if x != nil {
		return x.Auth
	}
	return nil
}

func (x *QueuesRequest) GetCompanyID() int64 {
	if x != nil {
		return x.CompanyID
	}
	return 0
}

type QueueRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Auth       *Auth  `protobuf:"bytes,1,opt,name=auth,proto3" json:"auth,omitempty"`
	CompanyID  int64  `protobuf:"varint,2,opt,name=companyID,proto3" json:"companyID,omitempty"`
	Code       string `protobuf:"bytes,3,opt,name=code,proto3" json:"code,omitempty"`
	Msv        string `protobuf:"bytes,4,opt,name=msv,proto3" json:"msv,omitempty"`
	GetAllJobs bool   `protobuf:"varint,5,opt,name=getAllJobs,proto3" json:"getAllJobs,omitempty"`
}

func (x *QueueRequest) Reset() {
	*x = QueueRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[17]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueueRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueueRequest) ProtoMessage() {}

func (x *QueueRequest) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[17]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueueRequest.ProtoReflect.Descriptor instead.
func (*QueueRequest) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{17}
}

func (x *QueueRequest) GetAuth() *Auth {
	if x != nil {
		return x.Auth
	}
	return nil
}

func (x *QueueRequest) GetCompanyID() int64 {
	if x != nil {
		return x.CompanyID
	}
	return 0
}

func (x *QueueRequest) GetCode() string {
	if x != nil {
		return x.Code
	}
	return ""
}

func (x *QueueRequest) GetMsv() string {
	if x != nil {
		return x.Msv
	}
	return ""
}

func (x *QueueRequest) GetGetAllJobs() bool {
	if x != nil {
		return x.GetAllJobs
	}
	return false
}

type QueueParamsSet struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Params *QueueParams `protobuf:"bytes,1,opt,name=params,proto3" json:"params,omitempty"`
	Code   string       `protobuf:"bytes,2,opt,name=code,proto3" json:"code,omitempty"`
	Msv    string       `protobuf:"bytes,3,opt,name=msv,proto3" json:"msv,omitempty"`
}

func (x *QueueParamsSet) Reset() {
	*x = QueueParamsSet{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[18]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueueParamsSet) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueueParamsSet) ProtoMessage() {}

func (x *QueueParamsSet) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[18]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueueParamsSet.ProtoReflect.Descriptor instead.
func (*QueueParamsSet) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{18}
}

func (x *QueueParamsSet) GetParams() *QueueParams {
	if x != nil {
		return x.Params
	}
	return nil
}

func (x *QueueParamsSet) GetCode() string {
	if x != nil {
		return x.Code
	}
	return ""
}

func (x *QueueParamsSet) GetMsv() string {
	if x != nil {
		return x.Msv
	}
	return ""
}

type QueueParams struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Period        int32 `protobuf:"varint,1,opt,name=period,proto3" json:"period,omitempty"`
	AutoReset     bool  `protobuf:"varint,2,opt,name=autoReset,proto3" json:"autoReset,omitempty"`
	AutoResetTime int32 `protobuf:"varint,3,opt,name=autoResetTime,proto3" json:"autoResetTime,omitempty"`
	WarningTime   int32 `protobuf:"varint,4,opt,name=warningTime,proto3" json:"warningTime,omitempty"`
}

func (x *QueueParams) Reset() {
	*x = QueueParams{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[19]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueueParams) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueueParams) ProtoMessage() {}

func (x *QueueParams) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[19]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueueParams.ProtoReflect.Descriptor instead.
func (*QueueParams) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{19}
}

func (x *QueueParams) GetPeriod() int32 {
	if x != nil {
		return x.Period
	}
	return 0
}

func (x *QueueParams) GetAutoReset() bool {
	if x != nil {
		return x.AutoReset
	}
	return false
}

func (x *QueueParams) GetAutoResetTime() int32 {
	if x != nil {
		return x.AutoResetTime
	}
	return 0
}

func (x *QueueParams) GetWarningTime() int32 {
	if x != nil {
		return x.WarningTime
	}
	return 0
}

type Queue struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Title       string       `protobuf:"bytes,1,opt,name=title,proto3" json:"title,omitempty"`
	Code        string       `protobuf:"bytes,2,opt,name=code,proto3" json:"code,omitempty"`
	Msv         string       `protobuf:"bytes,3,opt,name=msv,proto3" json:"msv,omitempty"`
	Processing  int32        `protobuf:"varint,4,opt,name=processing,proto3" json:"processing,omitempty"`
	Errors      bool         `protobuf:"varint,5,opt,name=errors,proto3" json:"errors,omitempty"`
	Warning     bool         `protobuf:"varint,6,opt,name=warning,proto3" json:"warning,omitempty"`
	LastDT      string       `protobuf:"bytes,7,opt,name=lastDT,proto3" json:"lastDT,omitempty"`
	Jobs        []*QueueJob  `protobuf:"bytes,8,rep,name=jobs,proto3" json:"jobs,omitempty"`
	JobsError   []*QueueJob  `protobuf:"bytes,9,rep,name=jobsError,proto3" json:"jobsError,omitempty"`
	JobsWarning []*QueueJob  `protobuf:"bytes,10,rep,name=jobsWarning,proto3" json:"jobsWarning,omitempty"`
	Params      *QueueParams `protobuf:"bytes,11,opt,name=params,proto3" json:"params,omitempty"`
}

func (x *Queue) Reset() {
	*x = Queue{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[20]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Queue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Queue) ProtoMessage() {}

func (x *Queue) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[20]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Queue.ProtoReflect.Descriptor instead.
func (*Queue) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{20}
}

func (x *Queue) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *Queue) GetCode() string {
	if x != nil {
		return x.Code
	}
	return ""
}

func (x *Queue) GetMsv() string {
	if x != nil {
		return x.Msv
	}
	return ""
}

func (x *Queue) GetProcessing() int32 {
	if x != nil {
		return x.Processing
	}
	return 0
}

func (x *Queue) GetErrors() bool {
	if x != nil {
		return x.Errors
	}
	return false
}

func (x *Queue) GetWarning() bool {
	if x != nil {
		return x.Warning
	}
	return false
}

func (x *Queue) GetLastDT() string {
	if x != nil {
		return x.LastDT
	}
	return ""
}

func (x *Queue) GetJobs() []*QueueJob {
	if x != nil {
		return x.Jobs
	}
	return nil
}

func (x *Queue) GetJobsError() []*QueueJob {
	if x != nil {
		return x.JobsError
	}
	return nil
}

func (x *Queue) GetJobsWarning() []*QueueJob {
	if x != nil {
		return x.JobsWarning
	}
	return nil
}

func (x *Queue) GetParams() *QueueParams {
	if x != nil {
		return x.Params
	}
	return nil
}

type Queues struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Data []*Queue `protobuf:"bytes,1,rep,name=data,proto3" json:"data,omitempty"`
}

func (x *Queues) Reset() {
	*x = Queues{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[21]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Queues) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Queues) ProtoMessage() {}

func (x *Queues) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[21]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Queues.ProtoReflect.Descriptor instead.
func (*Queues) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{21}
}

func (x *Queues) GetData() []*Queue {
	if x != nil {
		return x.Data
	}
	return nil
}

type QueueJob struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id        int64          `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	CompanyID int64          `protobuf:"varint,2,opt,name=companyID,proto3" json:"companyID,omitempty"`
	Shop      string         `protobuf:"bytes,3,opt,name=shop,proto3" json:"shop,omitempty"`
	Status    int32          `protobuf:"varint,4,opt,name=status,proto3" json:"status,omitempty"`
	Error     bool           `protobuf:"varint,5,opt,name=error,proto3" json:"error,omitempty"`
	Created   string         `protobuf:"bytes,6,opt,name=created,proto3" json:"created,omitempty"`
	Updated   string         `protobuf:"bytes,7,opt,name=updated,proto3" json:"updated,omitempty"`
	Logs      []*QueueJobLog `protobuf:"bytes,8,rep,name=logs,proto3" json:"logs,omitempty"`
}

func (x *QueueJob) Reset() {
	*x = QueueJob{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[22]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueueJob) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueueJob) ProtoMessage() {}

func (x *QueueJob) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[22]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueueJob.ProtoReflect.Descriptor instead.
func (*QueueJob) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{22}
}

func (x *QueueJob) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *QueueJob) GetCompanyID() int64 {
	if x != nil {
		return x.CompanyID
	}
	return 0
}

func (x *QueueJob) GetShop() string {
	if x != nil {
		return x.Shop
	}
	return ""
}

func (x *QueueJob) GetStatus() int32 {
	if x != nil {
		return x.Status
	}
	return 0
}

func (x *QueueJob) GetError() bool {
	if x != nil {
		return x.Error
	}
	return false
}

func (x *QueueJob) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *QueueJob) GetUpdated() string {
	if x != nil {
		return x.Updated
	}
	return ""
}

func (x *QueueJob) GetLogs() []*QueueJobLog {
	if x != nil {
		return x.Logs
	}
	return nil
}

type QueueJobLog struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Datetime string `protobuf:"bytes,1,opt,name=datetime,proto3" json:"datetime,omitempty"`
	Message  string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
}

func (x *QueueJobLog) Reset() {
	*x = QueueJobLog{}
	if protoimpl.UnsafeEnabled {
		mi := &file_board_v2_proto_msgTypes[23]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *QueueJobLog) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*QueueJobLog) ProtoMessage() {}

func (x *QueueJobLog) ProtoReflect() protoreflect.Message {
	mi := &file_board_v2_proto_msgTypes[23]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use QueueJobLog.ProtoReflect.Descriptor instead.
func (*QueueJobLog) Descriptor() ([]byte, []int) {
	return file_board_v2_proto_rawDescGZIP(), []int{23}
}

func (x *QueueJobLog) GetDatetime() string {
	if x != nil {
		return x.Datetime
	}
	return ""
}

func (x *QueueJobLog) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

var File_board_v2_proto protoreflect.FileDescriptor

var file_board_v2_proto_rawDesc = []byte{
	,
}

var (
	file_board_v2_proto_rawDescOnce sync.Once
	file_board_v2_proto_rawDescData = file_board_v2_proto_rawDesc
)

func file_board_v2_proto_rawDescGZIP() []byte {
	file_board_v2_proto_rawDescOnce.Do(func() {
		file_board_v2_proto_rawDescData = protoimpl.X.CompressGZIP(file_board_v2_proto_rawDescData)
	})
	return file_board_v2_proto_rawDescData
}

var file_board_v2_proto_msgTypes = make([]protoimpl.MessageInfo, 24)
var file_board_v2_proto_goTypes = []interface{}{
	(*PingService)(nil),                      // 0: cerasusV2.PingService
	(*PingServicesReply)(nil),                // 1: cerasusV2.PingServicesReply
	(*SearchProductRequest)(nil),             // 2: cerasusV2.SearchProductRequest
	(*BoardProduct)(nil),                     // 3: cerasusV2.BoardProduct
	(*BoardShopProduct)(nil),                 // 4: cerasusV2.BoardShopProduct
	(*BoardProductPricer)(nil),               // 5: cerasusV2.BoardProductPricer
	(*BoardProductCounterQueueData)(nil),     // 6: cerasusV2.BoardProductCounterQueueData
	(*BoardProductCounter)(nil),              // 7: cerasusV2.BoardProductCounter
	(*BoardProductData)(nil),                 // 8: cerasusV2.BoardProductData
	(*UpdateBoardCompanyServiceRequest)(nil), // 9: cerasusV2.UpdateBoardCompanyServiceRequest
	(*UpdateBoardCompanyRequest)(nil),        // 10: cerasusV2.UpdateBoardCompanyRequest
	(*BoardPricerShop)(nil),                  // 11: cerasusV2.BoardPricerShop
	(*BoardCounterData)(nil),                 // 12: cerasusV2.BoardCounterData
	(*BoardCompanyData)(nil),                 // 13: cerasusV2.BoardCompanyData
	(*BoardCompaniesRequest)(nil),            // 14: cerasusV2.BoardCompaniesRequest
	(*ReQueueRequest)(nil),                   // 15: cerasusV2.ReQueueRequest
	(*QueuesRequest)(nil),                    // 16: cerasusV2.QueuesRequest
	(*QueueRequest)(nil),                     // 17: cerasusV2.QueueRequest
	(*QueueParamsSet)(nil),                   // 18: cerasusV2.QueueParamsSet
	(*QueueParams)(nil),                      // 19: cerasusV2.QueueParams
	(*Queue)(nil),                            // 20: cerasusV2.Queue
	(*Queues)(nil),                           // 21: cerasusV2.Queues
	(*QueueJob)(nil),                         // 22: cerasusV2.QueueJob
	(*QueueJobLog)(nil),                      // 23: cerasusV2.QueueJobLog
	(*ShopProduct)(nil),                      // 24: cerasusV2.ShopProduct
	(*Company)(nil),                          // 25: cerasusV2.Company
	(*Brand)(nil),                            // 26: cerasusV2.Brand
	(*ShopData)(nil),                         // 27: cerasusV2.ShopData
	(*Auth)(nil),                             // 28: cerasusV2.Auth
	(*PingRequest)(nil),                      // 29: cerasusV2.PingRequest
	(*RequestByDates)(nil),                   // 30: cerasusV2.RequestByDates
	(*SearchRequest)(nil),                    // 31: cerasusV2.SearchRequest
	(*RequestByID)(nil),                      // 32: cerasusV2.RequestByID
	(*PingReply)(nil),                        // 33: cerasusV2.PingReply
	(*StatusReply)(nil),                      // 34: cerasusV2.StatusReply
	(*Count)(nil),                            // 35: cerasusV2.Count
	(*CompanyList)(nil),                      // 36: cerasusV2.CompanyList
}
var file_board_v2_proto_depIdxs = []int32{
	0,  // 0: cerasusV2.PingServicesReply.data:type_name -> cerasusV2.PingService
	24, // 1: cerasusV2.BoardShopProduct.shopProduct:type_name -> cerasusV2.ShopProduct
	6,  // 2: cerasusV2.BoardProductCounter.shopProductsCount:type_name -> cerasusV2.BoardProductCounterQueueData
	3,  // 3: cerasusV2.BoardProductData.product:type_name -> cerasusV2.BoardProduct
	25, // 4: cerasusV2.BoardProductData.company:type_name -> cerasusV2.Company
	26, // 5: cerasusV2.BoardProductData.brand:type_name -> cerasusV2.Brand
	4,  // 6: cerasusV2.BoardProductData.shopProducts:type_name -> cerasusV2.BoardShopProduct
	5,  // 7: cerasusV2.BoardProductData.pricerData:type_name -> cerasusV2.BoardProductPricer
	7,  // 8: cerasusV2.BoardProductData.counterData:type_name -> cerasusV2.BoardProductCounter
	25, // 9: cerasusV2.BoardCompanyData.company:type_name -> cerasusV2.Company
	27, // 10: cerasusV2.BoardCompanyData.shopData:type_name -> cerasusV2.ShopData
	11, // 11: cerasusV2.BoardCompanyData.pricerData:type_name -> cerasusV2.BoardPricerShop
	12, // 12: cerasusV2.BoardCompanyData.counterData:type_name -> cerasusV2.BoardCounterData
	28, // 13: cerasusV2.ReQueueRequest.auth:type_name -> cerasusV2.Auth
	28, // 14: cerasusV2.QueuesRequest.auth:type_name -> cerasusV2.Auth
	28, // 15: cerasusV2.QueueRequest.auth:type_name -> cerasusV2.Auth
	19, // 16: cerasusV2.QueueParamsSet.params:type_name -> cerasusV2.QueueParams
	22, // 17: cerasusV2.Queue.jobs:type_name -> cerasusV2.QueueJob
	22, // 18: cerasusV2.Queue.jobsError:type_name -> cerasusV2.QueueJob
	22, // 19: cerasusV2.Queue.jobsWarning:type_name -> cerasusV2.QueueJob
	19, // 20: cerasusV2.Queue.params:type_name -> cerasusV2.QueueParams
	20, // 21: cerasusV2.Queues.data:type_name -> cerasusV2.Queue
	23, // 22: cerasusV2.QueueJob.logs:type_name -> cerasusV2.QueueJobLog
	29, // 23: cerasusV2.Board.Ping:input_type -> cerasusV2.PingRequest
	16, // 24: cerasusV2.Board.GetQueues:input_type -> cerasusV2.QueuesRequest
	17, // 25: cerasusV2.Board.GetQueue:input_type -> cerasusV2.QueueRequest
	15, // 26: cerasusV2.Board.ReQueue:input_type -> cerasusV2.ReQueueRequest
	18, // 27: cerasusV2.Board.SetQueueParams:input_type -> cerasusV2.QueueParamsSet
	30, // 28: cerasusV2.Board.GetLogsCount:input_type -> cerasusV2.RequestByDates
	14, // 29: cerasusV2.Board.GetCompanies:input_type -> cerasusV2.BoardCompaniesRequest
	31, // 30: cerasusV2.Board.SearchCompanies:input_type -> cerasusV2.SearchRequest
	32, // 31: cerasusV2.Board.GetCompany:input_type -> cerasusV2.RequestByID
	10, // 32: cerasusV2.Board.UpdateCompany:input_type -> cerasusV2.UpdateBoardCompanyRequest
	9,  // 33: cerasusV2.Board.UpdateCompanyCounter:input_type -> cerasusV2.UpdateBoardCompanyServiceRequest
	9,  // 34: cerasusV2.Board.UpdateCompanyPricer:input_type -> cerasusV2.UpdateBoardCompanyServiceRequest
	2,  // 35: cerasusV2.Board.SearchProduct:input_type -> cerasusV2.SearchProductRequest
	29, // 36: cerasusV2.Board.PingServices:input_type -> cerasusV2.PingRequest
	33, // 37: cerasusV2.Board.Ping:output_type -> cerasusV2.PingReply
	21, // 38: cerasusV2.Board.GetQueues:output_type -> cerasusV2.Queues
	20, // 39: cerasusV2.Board.GetQueue:output_type -> cerasusV2.Queue
	34, // 40: cerasusV2.Board.ReQueue:output_type -> cerasusV2.StatusReply
	34, // 41: cerasusV2.Board.SetQueueParams:output_type -> cerasusV2.StatusReply
	35, // 42: cerasusV2.Board.GetLogsCount:output_type -> cerasusV2.Count
	36, // 43: cerasusV2.Board.GetCompanies:output_type -> cerasusV2.CompanyList
	36, // 44: cerasusV2.Board.SearchCompanies:output_type -> cerasusV2.CompanyList
	13, // 45: cerasusV2.Board.GetCompany:output_type -> cerasusV2.BoardCompanyData
	34, // 46: cerasusV2.Board.UpdateCompany:output_type -> cerasusV2.StatusReply
	34, // 47: cerasusV2.Board.UpdateCompanyCounter:output_type -> cerasusV2.StatusReply
	34, // 48: cerasusV2.Board.UpdateCompanyPricer:output_type -> cerasusV2.StatusReply
	8,  // 49: cerasusV2.Board.SearchProduct:output_type -> cerasusV2.BoardProductData
	1,  // 50: cerasusV2.Board.PingServices:output_type -> cerasusV2.PingServicesReply
	37, // [37:51] is the sub-list for method output_type
	23, // [23:37] is the sub-list for method input_type
	23, // [23:23] is the sub-list for extension type_name
	23, // [23:23] is the sub-list for extension extendee
	0,  // [0:23] is the sub-list for field type_name
}

func init() { file_board_v2_proto_init() }
func file_board_v2_proto_init() {
	if File_board_v2_proto != nil {
		return
	}
	file_cerasus_v2_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_board_v2_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PingService); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*PingServicesReply); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*SearchProductRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardProduct); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardShopProduct); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardProductPricer); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardProductCounterQueueData); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardProductCounter); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardProductData); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBoardCompanyServiceRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateBoardCompanyRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardPricerShop); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardCounterData); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardCompanyData); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[14].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*BoardCompaniesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[15].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ReQueueRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[16].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueuesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[17].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueueRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[18].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueueParamsSet); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[19].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueueParams); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[20].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Queue); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[21].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Queues); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[22].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueueJob); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_board_v2_proto_msgTypes[23].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*QueueJobLog); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_board_v2_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   24,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_board_v2_proto_goTypes,
		DependencyIndexes: file_board_v2_proto_depIdxs,
		MessageInfos:      file_board_v2_proto_msgTypes,
	}.Build()
	File_board_v2_proto = out.File
	file_board_v2_proto_rawDesc = nil
	file_board_v2_proto_goTypes = nil
	file_board_v2_proto_depIdxs = nil
}
